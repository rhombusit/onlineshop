...
class ConfirmPendingController extends Controller
{
...
//             ->where('users', 'users.c_firm_code', '=', 'order_mst.c_firm_code')
             ...
             ->where('order_mst.n_order_status', '=', '2')
             ->where('order_mst.c_firm_code', '=', '000000')
             ->orderBy('order_mst.n_srno', 'desc')
            ->get();
        return view('confirm_pending', compact('users'));
     return view('confirm_pending');
    }
 
   
     public function detailView(int $id2) {
         $users = DB::table('order_mst')
            ->join('users', 'users.id', '=', 'order_mst.c_user_code')
             ->select('users.c_firm_code','users.name', 'order_mst.n_srno', 'order_mst.d_date')
//             ->where('users', 'users.c_firm_code', '=', 'order_mst.c_firm_code')
             ...
             ->where('order_mst.n_order_status', '=', '2')
             ->where('order_mst.c_firm_code', '=', '000000')
             ->where('order_mst.n_srno', '=', $id2)->get();
        return view('DetailedViewPending', compact('users'));
    }
